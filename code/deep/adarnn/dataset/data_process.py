            ...
            for can in candidate:
                ...
                for i in range(1, len(selected)-1):
                    for j in range(i, len(selected)-1):
                        ...
                        dis_temp += criterion_transder.compute(torch.tensor(feat_part1, dtype=torch.float32), torch.tensor(feat_part2, dtype=torch.float32))
...
def load_weather_data_multi_domain(file_path, batch_size=6, station='Changping', number_domain=2, mode='pre_process', dis_type ='mmd'):

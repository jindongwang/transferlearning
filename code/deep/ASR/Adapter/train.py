...
def add_custom_arguments(parser):
    ...
    # AdapterFusion-related
    parser.add_argument("--adapter_fusion", type=str2bool, default=False)
...
def train_epoch(dataloader, model, optimizer, epoch=None):
    ...
    for batch_idx, data in enumerate(dataloader):
        ...
        if args.ngpu <= 1 or args.dist_train:
            ctc_att_loss, adapter_fusion_guide_loss = model(fbank, seq_lens, tokens, language)# .mean() # / self.accum_grad
        else:
            # apex does not support torch.nn.DataParallel
            ctc_att_loss, adapter_fusion_guide_loss = (
                data_parallel(model, (fbank, seq_lens, tokens, language), range(args.ngpu))# .mean() # / self.accum_grad
            )
        loss = ctc_att_loss.mean()
        if args.adapter_fusion:
            if hasattr(model, "module"):
                adapter_fusion_reg_loss = model.module.get_fusion_regularization_loss()
            else:
                adapter_fusion_reg_loss = model.get_fusion_regularization_loss()
            loss = loss + adapter_fusion_reg_loss
            stats["adapter_fusion_reg_loss_lst"].append(adapter_fusion_reg_loss.item())
            if args.guide_loss_weight > 0:
                if args.guide_loss_weight_decay_steps > 0:
                    ...
                    stats["adapter_fusion_guide_loss_weight"] = current_weight
                else:
                    current_weight = args.guide_loss_weight
                adapter_fusion_guide_loss = adapter_fusion_guide_loss.mean()
                loss = loss + current_weight * adapter_fusion_guide_loss
                stats["adapter_fusion_guide_loss_lst"].append(adapter_fusion_guide_loss.item())
...
def test(epoch, dataloader, model, model_path=None, language=None, visualize_adapter_fusion=False):
    if model_path:
    ...
    for batch_idx, data in enumerate(dataloader):
        ...
        if visualize_adapter_fusion:
            atts = model.calculate_adapter_fusion_attentions(fbank, seq_lens, tokens, language)
...
def train(dataloaders, model, optimizer, save_path):
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        valid_stats = test(f"val_{epoch}", val_loader, model, visualize_adapter_fusion=args.adapter_fusion)
...
if __name__ == "__main__":
    ...
    if len(args.adapter_train_languages) > 1 and not args.adapter_fusion and not args.shared_adapter:
        ...
        for idx, dataset in enumerate(args.dataset):
            ...
            for split in splits:
                ...
                test_stats = test(f"{split}_best", dataloaders[idx],
                                ...
                                save_path, 
                                language=language, 
                                visualize_adapter_fusion=args.adapter_fusion)

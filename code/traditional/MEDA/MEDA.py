...
class MEDA:
    def __init__(self, Xs, Ys, Xt, Yt, kernel_type='primal', dim=30, lamb=1, rho=1.0, eta=0.1, p=10, gamma=1, T=10):
    ...
    def fit_predict(self):
        gfk = GFK.GFK(self.Xs, self.Ys, self.Xt, self.Yt, dim=self.dim)
        _, Xs_new, Xt_new = gfk.fit()
        Xs_new, Xt_new = Xs_new.T, Xt_new.T
        X = np.hstack((Xs_new, Xt_new))
        n, m = Xs_new.shape[1], Xt_new.shape[1]
        C = len(np.unique(self.Ys))
        list_acc = []
        YY = np.zeros((n, C))
        ...
        knn_clf.fit(X[:, :n].T, self.Ys.ravel())
        ...
        for t in range(1, self.T + 1):
            mu = self.estimate_mu(Xs_new.T, self.Ys, Xt_new.T, Cls)
            ...
            for c in range(1, C + 1):
                e = np.zeros((n + m, 1))
                tt = Ys == c
                e[np.where(tt == True)] = 1 / len(self.Ys[np.where(self.Ys == c)])
                yy = Cls == c
